...
public class pngSuite {
	...
	//aux function to convert the HexString generated by the bytesToHexString to ASCII
    public String hexStringToASCII(String hex) {

        String retVal = "";

        for (int i = 0; i < hex.length(); i = i + 2) {
            if (Integer.parseInt(hex.substring(i, i + 2),16) >= 65 && Integer.parseInt(hex.substring(i, i + 2),16) <= 90 || Integer.parseInt(hex.substring(i, i + 2),16) >= 97 && Integer.parseInt(hex.substring(i, i + 2),16) <= 12) {
                retVal += (char)Integer.parseInt(hex.substring(i, i + 2),16);
            } else {
                return "";
            }
        }
    ...
    public ArrayList<chunk> pngParse(byte[] buffer) {
        ...
        while (base64ByteStringPointer < buffer.length) {
            ...
            if (isValidType(hexStringToASCII(bytesToHexString(Arrays.copyOfRange(buffer, base64ByteStringPointer, base64ByteStringPointer + 4))))) {
                System.out.println("found chunk starting at byte: " + (base64ByteStringPointer-4));
                if (hexStringToASCII(bytesToHexString(Arrays.copyOfRange(buffer, base64ByteStringPointer, base64ByteStringPointer + 4))).equals("IEND")) {
                    chunk end = new chunk();
                    end.setData(null);
                    end.setType("IEND");
                    end.setLen(0);
                ...
                String type = hexStringToASCII(bytesToHexString(Arrays.copyOfRange(buffer, base64ByteStringPointer, base64ByteStringPointer + 4)));
